...
class AlfredOutputFormatter:
    def __init__(self, items: list[AlfredItem]):
        self._items = items

    def print(self):
        output_format = {"items": self._items}
...
class LinkdingBookmark:
    def __init__(self, bookmark):
        ...
        self._title: str = (
            bookmark["title"] or bookmark["website_title"] or bookmark["url"]
        )
        self._tags: list[str] = list(
            map(lambda tag: "#%s" % tag, bookmark["tag_names"])
        )

    def to_alfred_item(self) -> AlfredItem:
        return AlfredItem(
            title=self._title, subtitle=self._url, arg=self._url, quicklookurl=self._url
        )

    def matches_query(self, query_token: list[str]) -> bool:
        if len(query_token) == 0:
            return True
        matched = list(
            map(lambda token: self._matches_single_token(token), query_token)
        )
        ...
        try:
            with open(bookmarks_cache_file, mode="r") as f:
                ...
                self._bookmarks = list(
                    map(lambda row: LinkdingBookmark(bookmark=row), bookmark_list)
                )
        except FileNotFoundError:
            self._bookmarks = []

    def bookmarks(self, query: str) -> list[LinkdingBookmark]:
        query = query.lower()
